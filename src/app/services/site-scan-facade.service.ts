...
export class SiteScanFacadeService {
  ...
  getMasterJSON(sName: string, fName: string): Observable<MasterScan> {
    if ( sName !== this.sitename || fName !== this.filename ) {
      this.masterJSON$ = this.siteScanAPIService.getMasterJSON( sName, fName ).pipe(shareReplay(1));
  ...
  getScanFile(sName: string, folderName: string, fName: string ): Observable<AxeScan> {
    if ( sName !== this.sitename || fName !== this.filename || folderName !== this.foldername ) {
      this.scanFile$ = this.siteScanAPIService.getScanFile( sName, folderName, fName ).pipe(shareReplay(1));
